...
{
    int size = C * L, ocup = 0, provDist, prevNode, auxPos;
    ...
    int *prev = (int *)malloc(size * sizeof(int)); // marcar o prev de outra maneira?
    if (prev == NULL)
    {
        free(dist);
        return 0;
    ...
    dist[0] = 0;
    ocup = HInsert(1, 1, size, ocup, HEAP, dist, C);

    while (HEAP[0] != NULL)
    {
        ocup = HRemove(&PresC, &PresL, ocup, HEAP, dist, C);
        ...
        if (maze[(PresL - 1) * C + PresC - 1] != 0)
        {
            ...
            if (provDist < dist[(NeighL - 1) * C + NeighC - 1])
            {
                ...
                j = inHeap(HEAP, &auxPos, NeighL, NeighC, ocup);
                if (j == 0)
                {
                    printf("%d\n", auxPos);
                    FixUp(HEAP, auxPos, dist, C);
                }
                else
                {
                    ocup = HInsert(NeighC, NeighL, size, ocup, HEAP, dist, C);
                }
            }
            continue;
        }
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            if (NeighState != 0)
            {
                if (A1(L, C, NeighL + VizL[i], NeighC + VizC[i], maze) != 0)
                {
                    continue;
                }
