    ...
    for ( l = 1; l <= L; l++)
    {
        for (c = 1; c <= C; c++)
        {
            if(maze[(l - 1) * C + c - 1]==0){
                ...
                while (stack != NULL)
                {
                    stack = read_pop_BFS(stack,&caux,&laux);
                    maze[(laux - 1) * C + caux - 1]=count;

                    if (caux == cend && laux == lend)
                    {
                        if (count==-3)
                        {
                            liberta_lista(stack);
                    ...
                    /*cima*/
                    if (A1(L, C, laux - 1, caux, maze) == 0)
                    {
                        insertNode = cria_no(caux, laux - 1);
                        stack = push(stack, insertNode);
                    }
                    /*baixo*/
                    if (A1(L, C, laux + 1, caux, maze) == 0)
                    ...
                    /*esquerda*/
                    if (A1(L, C, laux, caux - 1, maze) == 0)
                    {
                        insertNode = cria_no(caux - 1, laux);
                        stack = push(stack, insertNode);
                    }
                    /*direita*/
                    if (A1(L, C, laux, caux + 1, maze) == 0)
